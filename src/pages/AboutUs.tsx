...
const AboutUs = () => {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-[65] border-b border-border bg-background/80 backdrop-blur">
        <div className="mx-auto max-w-6xl px-6 h-16 flex items-center justify-between gap-4">
          <Link to="/" className="flex items-center gap-2" aria-label="Sedefy home">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm">
            ...
            <Link to="/pricing" className="text-muted-foreground hover:text-foreground transition-colors">{t("nav.pricing", "Pricing")}</Link>
            <Link to="/about-us" className="text-foreground font-medium">{t("about.badge")}</Link>
          </nav>
          <div className="flex items-center gap-2">
            <FloatingLanguageSelector />
            <Button asChild size="sm">
              <Link to="/auth">{t("nav.login", "Sign in")}</Link>
            </Button>
          </div>
        </div>
      </header>
